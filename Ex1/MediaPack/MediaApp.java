...
public class MediaApp {
    // MediaApp parms - the arrayList of the media
    ...
    /**
     * Function inputLength() - helper for 1
     * inputs a valid length if not valid, return -1
     */
    private static double inputLength()
    {
        Scanner length_scanner = new Scanner(System.in);

        System.out.println("Enter the length of the media in Minutes (in this form for example 5.34 or 3.0):");
        String input = length_scanner.nextLine();
        // we inputted as string, check the length is valid (should be able to make a double)
        double length = 0;
        try {
        ...
        // if we got here then the input is valid, now we need to check if this is valid time
        // Checking that the length really describes time (the number of seconds between 0 and 60)
        double decimal = length - (int)length;
        // check if the seconds is two-digit number
        String[] parts = input.split("\\.");
        if (length < 0 || decimal < 0.0 || decimal > 0.59 || parts.length != 2 || parts[1].length() > 2)
        {
            return -1;
        }
        return length;
    ...
    private static void playAllMedia()
    {
        if (media_list.isEmpty()){
            System.out.println("No Media yet");
            return;
        }
        for (Media media : media_list)
        {
